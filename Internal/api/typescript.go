...
// Moved to Internal/api/notuse/typescript.go
...
type TypeScriptPageData struct {
	SourceURL  string
	Posts      []model.Post
	IsLoggedIn bool
	CountPosts int
...
// Get_TypeScriptHandler serves the TypeScript page.
func (server *Server) Get_TypeScriptHandler(w http.ResponseWriter, r *http.Request) {
	countpost := 0
	isLoggedIn := false
	...
	catID, err := server.Service.GetCategoryIDByName("typescript")
	if err != nil {
	...
	if err := tpl.ExecuteTemplate(w, "TypeScript.html", data); err != nil {
		log.Printf("typescript: execute error: %v", err)
		server.Service.HandleError(w, http.StatusInternalServerError)
	}
}
